...
PACKAGE_NAME     := qubes-mgmt-salt-base-topd
...
FORMULA          ?= README.rst LICENSE topd _modules _utils
PILLAR           ?= 
TEST_FORMULA     ?= 
TEST_PILLAR      ?= 
...
.PHONY: install
install: 
	install -d -m 0750 $(DESTDIR)/$(SALT_STATE_DIR)/_topd
	install -d -m 0750 $(DESTDIR)/$(SALT_PILLAR_DIR)/_topd
